...
class it_equipment(osv.osv):
    ...
    def _virtual_count(self, cr, uid, ids, field_name, arg, context=None):
        res = dict(map(lambda x: (x,0), ids))
        # The current user may not have access rights
        try:
            for equipment in self.browse(cr, uid, ids, context):
                res[equipment.id] = len(equipment.virtual_ids)
        except:
            pass
        return res
    ...
    def _access_count(self, cr, uid, ids, field_name, arg, context=None):
        ...
        try:
            for equipment in self.browse(cr, uid, ids, context):
                res[equipment.id] = len(equipment.access_ids)
        except:
            pass
        return res
    ...
    def _backup_count(self, cr, uid, ids, field_name, arg, context=None):
        ...
        try:
            for equipment in self.browse(cr, uid, ids, context):
                res[equipment.id] = len(equipment.backup_ids)
        except:
            pass
        return res
    ...
    _columns = {
        ...
        'access_ids': fields.one2many('it.access', 'equipment_id','Access'),
        ...
        'virtual_count': fields.function(_virtual_count, string="Virtuals", type="integer"),
        ...
        'configuration_file_ids': fields.one2many('it.equipment.configuration','equipment_id','Configuration Files'),
